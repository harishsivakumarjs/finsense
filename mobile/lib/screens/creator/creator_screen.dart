...
class _CreatorScreenState extends ConsumerState<CreatorScreen> {
        ...
        child: async.when(
          ...
          data: (entries) {
            ...
            return ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 16), children: [
              Row(children: [
                Expanded(child: FSMetricCard(label: 'AdSense', value: adsense, accentColor: c.warning)),
                const SizedBox(width: 10),
                Expanded(child: FSMetricCard(label: 'Sponsorships', value: sponsorships, accentColor: c.purple)),
              ]).animate().fadeIn(),
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: FSMetricCard(label: 'Memberships', value: memberships, accentColor: c.info)),
                const SizedBox(width: 10),
                Expanded(child: FSMetricCard(label: 'Total', value: total, accentColor: c.positive)),
              ]).animate(delay: 80.ms).fadeIn(),
              const SizedBox(height: 16),
              Text('ALL ENTRIES', style: GoogleFonts.plusJakartaSans(fontSize: 10, fontWeight: FontWeight.w600, color: c.textTertiary, letterSpacing: 0.8)),
  ...
  Future<void> _delete(String id) async => ref.read(creatorProvider.notifier).remove(id);
...
class _CreatorRow extends StatelessWidget {
  ...
  Color _typeColor(BuildContext context) {
    final c = context.fsc;
    return switch (entry.incomeType) {
      'adsense' => c.warning,
      'sponsorship' => c.purple,
      'membership' => c.info,
      'merch' => c.teal,
      'course' => c.positive,
      _ => c.textSecondary,
        ...
        child: Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                entry.brandName ?? entry.videoTitle ?? entry.incomeType,
            ...
            Text(formatINR(entry.amount), style: GoogleFonts.jetBrainsMono(fontSize: 13, color: c.positive, fontWeight: FontWeight.w500)),
          ]),
        ),
      ),
    );
...
class _AddCreatorSheetState extends ConsumerState<_AddCreatorSheet> {
  ...
  void initState() {
    ...
    if (widget.entry != null) {
      ...
      _date = DateTime.tryParse(e.receivedOn) ?? DateTime.now();
    }
  }
